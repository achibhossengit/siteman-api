...
class SiteDailyRecordAPITestCase(APITestCase):
    ...
    def _by_date_url(self, site_id, record_date):
        return reverse(
            "site-daily-record-by-date",
            kwargs={
                "version": "v1",
                "site_pk": site_id,
    ...
    def _pending_log_url(self, site_id, record_date):
        return reverse(
            "site-daily-record-pending-log",
            kwargs={
                "version": "v1",
                "site_pk": site_id,
                "record_date": str(record_date),
...
class SiteDailyRecordByDateTests(SiteDailyRecordAPITestCase):
    def test_lists_records_for_date_without_pagination(self):
        today = timezone.localdate()
        yesterday = today - timedelta(days=1)
        first = self._create_daily_record(labour=self.labour, date=today)
        second = self._create_daily_record(labour=self.labour_b, date=today)
        self._create_daily_record(
            labour=self.labour,
            date=yesterday,
        )
    ...
    def test_empty_list_when_no_records_for_date(self):
        self._create_daily_record(date=timezone.localdate() - timedelta(days=1))
...
class SiteDailyRecordPendingLogTests(SiteDailyRecordAPITestCase):
    def _create_daily_record_log(
        self, *, site=None, business_date=None, reviewed=False, **kwargs
    ):
        site = site or self.site
        defaults = {
            "company": site.company,
    ...
    def test_lists_pending_daily_record_logs_without_pagination(self):
        today = timezone.localdate()
        yesterday = today - timedelta(days=1)
        pending = self._create_daily_record_log(business_date=today, entity_id=10)
        self._create_daily_record_log(
            business_date=today, entity_id=11, reviewed=True
        )
        self._create_daily_record_log(business_date=yesterday, entity_id=12)
        self._create_daily_record_log(
            business_date=today,
            entity_id=13,
        ...
        self.assertEqual(
            response.data[0]["entity_type"], ActivityEntityType.DAILY_RECORD
        )
        self.assertIsNone(response.data[0]["reviewed_at"])

    def test_empty_list_when_no_pending_logs(self):
        self._create_daily_record_log(reviewed=True)
        response = self.client.get(
    ...
    def test_other_site_logs_hidden(self):
        ...
        self._assign_site(self.user, other_site)
        self._create_daily_record_log(site=other_site)
        response = self.client.get(
            self._pending_log_url(self.site.pk, timezone.localdate())
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_invalid_calendar_date_returns_400(self):
        response = self.client.get(
            self._pending_log_url(self.site.pk, "2026-02-30")
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["errors"][0]["code"],
