...
| ------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- |
| `POST` | `/api/v1/auth/register`                  | Validate name/phone/email/company/password; stash payload + OTP in Redis; return ticket                                         | MVP      | done     |
| `POST` | `/api/v1/auth/register/resend-otp`       | Resend OTP for ticket (60s cooldown, max 5/hr); regenerate code, drop old                                                       | MVP      | done     |
| `POST` | `/api/v1/auth/register/confirm`          | Verify OTP; one tx: create company + admin user + Company Admin group + seed trial subscription; auto-login (issue JWT)         | MVP      | done     |
| `POST` | `/api/v1/auth/token/obtain`              | Phone + password; validate BD phone + active account/company; issue JWT access + refresh (refresh token set in httponly cookie) | MVP      | done     |
| `POST` | `/api/v1/auth/token/refresh`             | Exchange refresh for new access; rotate refresh                                                                                 | MVP      | done     |
| `POST` | `/api/v1/auth/token/blacklist`           | Blacklist current refresh token in database                                                                                     | MVP      | done     |
| `POST` | `/api/v1/auth/password/reset`            | Registered phone; always 200 (no enumeration); send OTP; return ticket                                                          | MVP      | done     |
| `POST` | `/api/v1/auth/password/reset/resend-otp` | Resend reset OTP for ticket (cooldown)                                                                                          | MVP      | done     |
| `POST` | `/api/v1/auth/password/reset/confirm`    | Verify OTP {ticket, code, new_password}; set password; bump token_version (invalidate all tokens)                               | MVP      | done     |
| `POST` | `/api/v1/auth/password/change`           | Logged-in: current + new password; bump token_version then re-issue current pair (kills all other tokens)                       | MVP      | done     |
...
| Method   | Endpoint                                       | Description                                                          | Priority | Progress |
| -------- | ---------------------------------------------- | -------------------------------------------------------------------- | -------- | -------- |
| `GET`    | `/api/v1/company/profile`                      | View company profile & status (name/active/site count/plan/validity) | L1       | pending  |
| `PATCH`  | `/api/v1/company/profile`                      | Edit company name & profile; activity-logged                         | L1       | pending  |
| `GET`    | `/api/v1/company/config`                       | View CompanyConfig (allow_labour_transfer/auto_renew)                | L1       | pending  |
...
| `GET`    | `/api/v1/company/custom-categories`            | List custom categories filtered by scope                             | L1       | pending  |
...
| Method   | Endpoint                         | Description                                                                                                                                                                | Priority | Progress |
| -------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- |
| `GET`    | `/api/v1/users`                  | List/search company users; filters role/site/active                                                                                                                        | L2       | pending  |
| `POST`   | `/api/v1/users`                  | Create staff (name/phone/role/sites); validate phone + check user limit/permissions; send OTP to new user's phone; stash payload in Redis; return ticket (no user row yet) | L2       | pending  |
| `POST`   | `/api/v1/users/resend-otp`       | Resend create OTP for ticket (60s cooldown, max 5/hr)                                                                                                                      | L2       | pending  |
| `POST`   | `/api/v1/users/verify-otp`       | Verify OTP {ticket, code}; create user (is_staff=false) + assign role/sites; auto-gen random password; SMS it to user ("login with this, reset it, don't share")           | L2       | pending  |
| `GET`    | `/api/v1/users/{id}`             | View user detail                                                                                                                                                           | L2       | pending  |
| `PATCH`  | `/api/v1/users/{id}`             | Admin edit name (instant) / phone/email (held in Redis pending OTP to user's phone, return ticket)                                                                         | L2       | pending  |
| `POST`   | `/api/v1/users/{id}/verify-otp`  | Verify OTP {ticket, code}; apply pending detail change                                                                                                                     | L2       | pending  |
| `POST`   | `/api/v1/users/{id}/roles`       | Assign/change role (group); union capability                                                                                                                               | L3       | pending  |
| `POST`   | `/api/v1/users/{id}/sites`       | Assign user to multiple sites (array payload); UserSite links                                                                                                              | L3       | pending  |
| `DELETE` | `/api/v1/users/{id}/sites`       | Unassign user from multiple sites (array payload)                                                                                                                          | L3       | pending  |
| `POST`   | `/api/v1/users/{id}/permissions` | Grant/revoke fine-grained permissions                                                                                                                                      | L3       | pending  |
| `POST`   | `/api/v1/users/{id}/activate`    | Set is_active=true                                                                                                                                                         | L3       | pending  |
| `POST`   | `/api/v1/users/{id}/deactivate`  | Set is_active=false                                                                                                                                                        | L3       | pending  |
| `DELETE` | `/api/v1/users/{id}`             | Soft-delete user (deleted_at); RESTRICT if referenced                                                                                                                      | L4       | pending  |
| `POST`   | `/api/v1/users/{id}/restore`     | Restore soft-deleted user (clear deleted_at)                                                                                                                               | L4       | pending  |
...
| Method   | Endpoint                                           | Description                                                                                                                    | Priority | Progress |
| -------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------- | -------- |
| `GET`    | `/api/v1/sites`                                    | List sites; filters open/closed active/inactive; scoped by assignment                                                          | MVP      | done     |
| `POST`   | `/api/v1/sites`                                    | Create site (open+active); checks open-site limit                                                                              | MVP      | done     |
| `GET`    | `/api/v1/sites/{id}`                               | View site detail                                                                                                               | MVP      | done     |
| `PATCH`  | `/api/v1/sites/{id}`                               | Edit site name/fields; handle activate, deactivate partial operation                                                           | MVP      | done     |
| `DELETE` | `/api/v1/sites/{id}`                               | Delete site only if no financial records / refs                                                                                | L1       | done     |
...
| `GET`    | `/api/v1/sites/{id}/config`                        | View SiteConfig (windows/quotas/validation ranges)                                                                             | L1       | pending  |
...
| `GET`    | `/api/v1/sites/{id}/billing-categories`            | List site billing categories; filter by sites, etc                                                                             | L1       | pending  |
...
| Method  | Endpoint               | Description                                                                                                               | Priority | Progress |
| ------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------- | -------- | -------- |
| `GET`   | `/api/v1/labours`      | List/search labour; filters site/active, current_site, etc;                                                               | MVP      | done     |
| `POST`  | `/api/v1/labours`      | Create labour (name/defaults/site); checks labour limit & name uniqueness                                                 | MVP      | done     |
| `GET`   | `/api/v1/labours/{id}` | View labour detail                                                                                                        | MVP      | done     |
| `PATCH` | `/api/v1/labours/{id}` | Edit labour fields/defaults; handle activate(is_active=true), deactivate and transfer(current_site change) labour account | MVP      | done     |
| `DELETE` | `/api/v1/labours/{id}` | allow delete when balance = 0, which implement after all record entity creation | L1      | pending     |
...
| Method   | Endpoint                         | Description                                                                                                                                                                                                 | Priority | Progress |
| -------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- |
| `GET`    | `/api/v1/labour-payments`        | List payments; filters labour, site, type (advance\|fooding), sealed/non-sealed                                                                                                                             | MVP      | pending  |
| `POST`   | `/api/v1/labour-payments`        | Create payment(s); `type` required; fooding amount seeded from `default_fooding`; body always array (single = array of 1, hajira sheet = many); one tx per request; blocked if labour inactive; recompute type-scoped `site_total` once; returns {created, errors[]} | MVP      | pending  |
| `PATCH`  | `/api/v1/labour-payments/{id}`   | Edit unsealed row in window; activity-logged; recompute totals                                                                                                                                              | L1       | pending  |
| `DELETE` | `/api/v1/labour-payments/{id}`   | Soft-delete unsealed row in window; activity-logged; recompute totals                                                                                                                                       | L1       | pending  |
...
| `GET`  | `/api/v1/reports/site-labour-cost`         | Attendance salary/extra/`labour_payment` (advance+fooding) per site & category | L1       | pending  |
