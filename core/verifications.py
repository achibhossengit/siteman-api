...
def _save(ticket, data):
    ...
    if remaining <= 0:
        ...
        raise ValidationError(code=status_codes.EXPIRED, detail={"ticket": "This verification ticket has expired."})
...
def resend(ticket, purpose=None):
    ...
    if elapsed < RESEND_COOLDOWN:
        ...
        raise Throttled(wait=math.ceil(RESEND_COOLDOWN - elapsed), code=status_codes.RESEND_COOLDOWN, detail="Please wait before requesting a new code.")
    if data["resend_count"] >= MAX_RESENDS:
        raise Throttled(code=status_codes.MAX_RESENDS, detail="Too many code requests. Try again later.")
...
def verify(ticket, code, purpose=None):
    ...
    if data["attempts"] >= MAX_ATTEMPTS:
        cache.delete(_key(ticket))
        raise ValidationError(code=status_codes.MAX_ATTEMPTS, detail="Too many invalid attempts. Request a new code.")
    if _now() > data["otp_expires_at"]:
        raise ValidationError(code=status_codes.EXPIRED, detail={"otp": "This OTP has expired. Request a new one."})
    if not check_password(code, data["otp_hash"]):
        data["attempts"] += 1
        _save(ticket, data)
        raise ValidationError(code=status_codes.INVALID, detail={"otp": "Invalid verification OTP."})
    cache.delete(_key(ticket))
    return data["payload"]
